...
generate_prompt_from_csv = False
...
def generate_prompt(src, tgt, input_data_path):
    prompt = f'{src}'
    df = pd.read_csv(input_data_path)
    for i in range(len(df)):
        prompt = prompt + f': {df.loc[i,src]}\n{tgt}: {df.loc[i,tgt]}\n####\n{src}'
    return prompt

#if __name__ == '__main__':


async def convert_code(src, tgt, query, category):
    ...
    if generate_prompt_from_csv:
        prompt = generate_prompt(src, tgt, data['codetranslation']['input_data_path'])
        #print(prompt)
    else:
